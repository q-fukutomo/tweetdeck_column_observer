...
カスタマイズ
設定ページを作っていないのでソースを直接編集して対応する形になります(ごめんなさい)
...
監視タグを変えたい
→ content.jsの一番上でタグ用の変数があるのでそこを編集、先頭の#は不要
...
文字色変えたい
→ content.cssの #tweetTextBox { 〜 }の中のcolorの右辺をいじる、黒くしたければ#000000とか16進数形式のカラーコード渡す

クロマキー用の背景色変えたい
→ content.cssの #tweetTextBox { 〜 }の中のbackground-colorの右辺をいじる、右辺は文字色と同じ形式で指定

テキストボックスの縁と文字の距離調整したい
...
※先頭から順番にそのフォントあるか判定してあれば適用、なければ次のフォント名で〜って繰り返す仕様らしい
